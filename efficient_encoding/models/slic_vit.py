...
class SLICViT(nn.Module):
    ...
    def verify(self, image_features_normalized, text, root_path):
        with torch.no_grad():
            ...
            for i in range(r):
                for j in range(c):
                    query_map[i,j,0] = (torch.dot(image_features_normalized[i,j,:], text_features_normalized).cpu()/ (np.linalg.norm(image_features_normalized[i,j,:].cpu().detach().numpy()) * np.linalg.norm(text_features_normalized.cpu().detach().numpy())))
        return query_map
