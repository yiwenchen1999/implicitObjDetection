...
from clip.clip import _MODELS, _download, available_models, _convert_image_to_rgb
...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu",
         jit: bool = False, download_root: str = None, high_res: bool = False, neighbor_window=None, split_layer=-1):
    """Load a Spatial CLIP model
    ...
    if not jit:
        model = build_spatial_model(state_dict or model.state_dict(
        ), high_res=high_res, neighbor_window=neighbor_window, split_layer=split_layer).to(device)
        if str(device) == "cpu":
            model.float()
