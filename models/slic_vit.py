...
class SLICViT(nn.Module):
    ...
    def verify(self, image_features, text_features):
        ...
        for i in range(r):
            for j in range(c):
                query_map[i,j,0] = np.dot(image_features_normalized[i,j,:], text_features_normalized) / ((np.linalg.norm(image_features_normalized[i,j,:]) * np.linalg.norm(text_features_normalized)))
        return query_map
