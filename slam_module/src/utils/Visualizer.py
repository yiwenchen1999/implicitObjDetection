...
class Visualizer(object):
    ...
    def vis(self, idx, iter, gt_depth, gt_color,gt_belief, c2w_or_camera_tensor, c,
            decoders):
        """
        ...
        with torch.no_grad():
            if (idx % self.freq == 0) and (iter % self.inside_freq == 0):
                ...
                gt_belief_np = gt_belief.cpu().numpy()
                ...
                depth, uncertainty, belief = self.renderer.render_img(
                   c,
                   decoders,
                   c2w,
                   self.device,
                ...
                gt_belief_np = np.clip(gt_belief_np, 0, 1)
                axs[0, 2].imshow(gt_belief_np, cmap="plasma",
                                 vmin=0, vmax=max_depth)
                ...
                belief_np = np.clip(belief_np, 0, 1)
                ...
                axs[1, 1].set_xticks([])
                axs[1, 1].set_yticks([])
                axs[1, 2].imshow(belief_np, cmap="plasma")
                axs[1, 2].set_title('beliefs')
                axs[1, 2].set_xticks([])
                axs[1, 2].set_yticks([])
