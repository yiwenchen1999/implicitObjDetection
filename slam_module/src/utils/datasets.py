...
class BaseDataset(Dataset):
    def __init__(self, cfg, args, scale, device='cuda:0'
        ...
        # print(device)
        # self.model_clip, self.preprocess = clip.load("ViT-B/32", device=device)
    ...
    def __getitem__(self, index):
        ...
        belief_path = '/gpfs/data/ssrinath/ychen485/implicitSearch/implicitObjDetection/output0/'+str(int(index))+"_heat.png"
        belief_data = cv2.imread(belief_path)
        ...
        belief_data = cv2.cvtColor(color_data, cv2.COLOR_BGR2RGB)
        color_data = color_data / 255.
        # belief_data = belief_data/255.
        depth_data = depth_data.astype(np.float32) / self.png_depth_scale
        H, W = depth_data.shape
        color_data = cv2.resize(color_data, (W, H))
        belief_data = cv2.resize(belief_data, (W, H))
        color_data = torch.from_numpy(color_data)
        belief_data = torch.from_numpy(belief_data)
        ...
        if self.crop_size is not None:
            ...
            belief_data = belief_data.permute(2, 0, 1)
            belief_data = F.interpolate(
                belief_data[None], self.crop_size, mode='bilinear', align_corners=True)[0]
            belief_data = belief_data.permute(1, 2, 0).contiguous()
        ...
        if edge > 0:
            ...
            belief_data = belief_data[edge:-edge, edge:-edge]
        pose = self.poses[index]
        pose[:3, 3] *= self.scale
        
        # clip_input = self.preprocess(clip_input).unsqueeze(0).to(self.device)
        # target_text = "a white board"
        # text = clip.tokenize(target_text).to(self.device)
        # clip_score = evaluateFrame(clip_input, text, self.model_clip)
        return index, color_data.to(self.device), depth_data.to(self.device), pose.to(self.device), belief_data.to(self.device)
