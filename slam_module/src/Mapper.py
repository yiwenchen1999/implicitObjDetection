...
from clip_score import evaluateFrame
...
class Mapper(object):
    ...
    def keyframe_selection_overlap(self, gt_color, gt_depth, c2w, gt_clip, keyframe_dict, k, N_samples=16, pixels=100):
        """
        ...
        rays_o, rays_d, gt_depth, gt_color, _ = get_samples(
            0, H, 0, W, pixels, H, W, fx, fy, cx, cy, c2w, gt_depth, gt_color, gt_clip, self.device)
    ...
    def optimize_map(self, num_joint_iters, lr_factor, idx, cur_gt_color, cur_gt_depth, gt_cur_c2w, cur_gt_clip, keyframe_dict, keyframe_list, cur_c2w):
        """
        ...
        if len(keyframe_dict) == 0:
            ...
            if self.keyframe_selection_method == 'global':
                ...
                optimize_frame = self.keyframe_selection_overlap(
                    cur_gt_color, cur_gt_depth, cur_c2w, cur_gt_clip, keyframe_dict[:-1], num)
        ...
        #------------------getting the data that needs to be optimized-------------------
        if self.nice: 
            ...
            for key, val in c.items():
                ...
                # print(key, val.shape)
                # print("frustum_feature_selection" + str(self.frustum_feature_selection))
                if not self.frustum_feature_selection:
                    ...
                    if key == 'grid_coarse':
                        ...
                        clip_grid_para.append(val_grad)
        #----------------------------------------------------------------------------------

        #----------------get parameters that needs to be optimized-------------------------
        if self.nice:
            ...
            if not self.fix_color:
                ...
                decoders_para_list += list(
                    self.decoders.clip_decoder.parameters())
        else:
            # imap*, single MLP
        ...
        print("num_joint_iters in optimize is: " + str(num_joint_iters))
        #-----------------back prop for num_joint_iters--------------------------------------
        for joint_iter in range(num_joint_iters):
            ...
            if self.nice:
                ...
                optimizer.param_groups[5]['lr'] = cfg['mapping']['stage'][self.stage]['color_lr']*lr_factor
            ...
            if (not (idx == 0 and self.no_vis_on_first_frame)) and ('Demo' not in self.output):
                self.visualizer.vis(
                    idx, joint_iter, cur_gt_depth, cur_gt_color, cur_gt_clip, cur_c2w, self.c, self.decoders)
            ...
            for frame in optimize_frame:
                #---------------get ground truth depth&color-------------------------
                if frame != -1:#not current frame
                    gt_depth = keyframe_dict[frame]['depth'].to(device)
                    gt_color = keyframe_dict[frame]['color'].to(device)
                    gt_clip = keyframe_dict[frame]['clip'].to(device)
                ...
                #--------------sample rays+ground truth outcome-----------------------
                batch_rays_o, batch_rays_d, batch_gt_depth, batch_gt_color, batch_gt_clip = get_samples(
                    0, H, 0, W, pixs_per_image, H, W, fx, fy, cx, cy, c2w, gt_depth, gt_color, gt_clip, self.device)
                ...
                # batch_gt_belief = gt_belief.repeat(batch_gt_color.shape)
                batch_gt_clip_list.append(batch_gt_clip.float())
                #-------------------------------------------------------------
            ...
            batch_gt_clip = torch.cat(batch_gt_clip_list)
            ...
            if self.nice:
                ...
                batch_gt_clip = batch_gt_clip[inside_mask]
            #-----------------------getting outcome of the ray from renderer----------------
            ...
            if self.stage == 'color':
                # print("color loss: "+ str(loss))
                ret = self.renderer.render_batch_ray(c, self.decoders, batch_rays_d,
                                                    batch_rays_o, device, 'clip',
                                                     gt_depth=None if self.coarse_mapper else batch_gt_depth)
                depth, uncertainty, clip = ret

                depth_mask = (batch_gt_depth > 0)
                if ((not self.nice) or (self.stage == 'color')):
                    clip_loss = torch.abs(batch_gt_clip - clip).sum()
                    weighted_clip_loss = self.w_color_loss*clip_loss*3
                    loss += weighted_clip_loss
                # print("belief loss: "+ str(loss))
    ...
    def run(self):
        ...
        cfg = self.cfg
        idx, gt_color, gt_depth, gt_c2w, color_path = self.frame_reader[0]
        print("read from frame reader:")
        print(type(idx),type(gt_color),type(gt_depth),type(gt_c2w))
        print(idx,gt_color.shape,gt_depth.shape,gt_c2w.shape)
        # evaluateFrame(clip_input, "a whiteboard")

        # clip_input = Image.open(color_path)
        # clip_input = self.preprocess(clip_input).unsqueeze(0).to(self.device)
        # target_text = "an office chair"
        # text = clip.tokenize(target_text).to(self.device)
        # clip_score = evaluateFrame(clip_input, text, self.model_clip)

        # irrelavant_text = "a bird in tropical forest"
        # irrelavant_text = clip.tokenize(irrelavant_text).to(self.device)
        # self.low_thr = evaluateFrame(clip_input, irrelavant_text, self.model_clip)
        # clip_score = clip_score - self.low_thr
        # clip_score[clip_score<0] = 0
        ...
        while (1):
            ...
            #------------------reading data----------------------------------------
            _, gt_color, gt_depth, gt_c2w, gt_clip = self.frame_reader[idx]
            ...
            # clip_score = clip_score - self.low_thr
            # clip_score[clip_score<0] = 0
            # print("current clip score is: " + str(clip_score))


            # print("color image: ")
            # print(type(gt_color), gt_color.shape)
            # evaluateFrame(clip_input, "a white board")

            # print("getting from idx:" + str(idx))
            # print(type(gt_color),type(gt_depth),type(gt_c2w))
            #-----------------------------------------------------------------------


            #---------------need to figure out what is outer_joint_iters-------------
            if not init:
            ...
            cur_c2w = self.estimate_c2w_list[idx].to(self.device)# getting cam pose
            num_joint_iters = num_joint_iters//outer_joint_iters
            # print("num_joint_iters: " + str(num_joint_iters))
            # print("outer_joint_iters: " + str(outer_joint_iters))

            #---------------optimization steps-----------------------------------------
            for outer_joint_iter in range(outer_joint_iters):

                #decide to do bundle adjustment if keyframe>4
                ...
                _ = self.optimize_map(num_joint_iters, lr_factor, idx, gt_color, gt_depth,
                                      gt_c2w, gt_clip, self.keyframe_dict, self.keyframe_list, cur_c2w=cur_c2w)
                ...
                if outer_joint_iter == outer_joint_iters-1:
                    if (idx % self.keyframe_every == 0 or (idx == self.n_img-2)) \
                        ...
                        ), 'depth': gt_depth.cpu(), 'est_c2w': cur_c2w.clone(), 'clip': gt_clip.cpu()})
            #----------------------------------------------------------------------------
