...
def select_uv(i, j, n, depth, color, belief, device='cuda:0'):
    """
    ...
    belief = belief.reshape(-1,3)
    depth = depth[indices]  # (n)
    color = color[indices]  # (n,3)
    belief = belief[indices]
    return i, j, depth, color, belief


def get_sample_uv(H0, H1, W0, W1, n, depth, color, belief, device='cuda:0'):
    """
    ...
    i = i.t()  # transpose
    j = j.t()
    i, j, depth, color, belief = select_uv(i, j, n, depth, color, belief, device=device)
    return i, j, depth, color, belief


def get_samples(H0, H1, W0, W1, n, H, W, fx, fy, cx, cy, c2w, depth, color, belief, device):
    """
    Get n rays from the image region H0..H1, W0..W1.
    c2w is its camera pose and depth/color is the corresponding image tensor.

    """
    i, j, sample_depth, sample_color, sample_belief = get_sample_uv(
        H0, H1, W0, W1, n, depth, color, belief, device=device)
    rays_o, rays_d = get_rays_from_uv(i, j, c2w, H, W, fx, fy, cx, cy, device)
    return rays_o, rays_d, sample_depth, sample_color, sample_belief
