...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    if kwargs['train_clip']:
        k_extract = ['clip_map', 'disp_map', 'acc_map']
    else:
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False, outputClip = False, infer = False, raw_rgb = None):
    ...
    if infer:
        ...
        result = (alpha_clip > 0.5) * alpha_clip #attempt to threshold alpha
        alpha_clip = result #attempt to threshold alpha
...
def train():
    ...
    os.makedirs(os.path.join(basedir, expname+"_clip"), exist_ok=True)
